...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.m-cascader-node[tabindex="-1"]');
...
export default {
  ...
  methods: {
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.m-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.m-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    handleExpand(node, silent) {
      ...
      //console.log("handleExpand",node,silent)
      if (!node.isLeaf) {
        path.push(node);
    ...
    */
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.m-scrollbar__wrap');
          const activeNode = menuElement.querySelector('.m-cascader-node.is-active') ||
            menuElement.querySelector('.m-cascader-node.in-active-path');
