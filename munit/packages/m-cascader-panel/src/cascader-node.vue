  ...
  export default {
    ...
    methods: {
      ...
      handleMultiCheckChange(checked) {
        let curchecked;
          (typeof checked == "boolean") ? curchecked = checked
          : curchecked = checked.target.checked
        
        //console.log(checked,"handleMultiCheckChange")
        this.node.doCheck(curchecked);
      ...
      renderPrefix(h) {
        ...
        // isChecked = true
        // isLeaf = true
        //console.log(isLeaf,isChecked,123)
        if (multiple) {
      ...
      renderCheckbox(h) {
        ...
        //   curchecked = node.checked;
        // }else{
        //   curchecked = node.checked.target.checked;
        // }
        // (typeof node.checked  == "boolean") ?  curchecked = node.checked : curchecked = true
      ...
      renderCheckIcon(h) {
        return ( //对号
          <a-icon type="check" class="m-cascader-node__prefix" />
        );
      },

      renderLoadingIcon(h) {
        return ( //加载
          <a-spin size="small" class="m-cascader-node__postfix" />
        );
      },

      renderExpandIcon(h) {
        return ( //箭头
          <a-icon type="right" class="m-cascader-node__postfix" />
        );
      },
